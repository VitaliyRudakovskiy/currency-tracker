...
import { setCurrenciesToStore, setExchangeRate, setUpdateTime } from '@store/reducers/currencySlice';
	...
	useEffect(() => {
		const fetchData = async () => {
			try {
				...
				const currenciesFromApi: ICurrency[] = Object.keys(currencyNames).map((code) => ({
					code,
