describe('Contacts Page E2E Test', () => {
	...
	it('should be able change theme', () => {
		...
		cy.get('[data-cy=contacts-wrapper]').should('have.css', 'background-color', 'rgb(4, 4, 4)');
		cy.get('[data-cy=theme-toggler]').click();
		cy.get('[data-cy=contacts-wrapper]').should('have.css', 'background-color', 'rgb(244, 255, 244)');
	});
